use std::{cmp::min, fmt::Debug, iter::empty};

use opencv::{
    core::{in_range, BorderTypes, Mat, Scalar, Size, VecN, CV_8UC1}, highgui, imgproc::{find_contours, gaussian_blur, ContourApproximationModes, RetrievalModes}, types::VectorOfVectorOfPoint
};
use rand::Rng;

use crate::{
    config::colours, points::{Point, PointMap, PointType, Pos}
};
...
impl ObjectFinder for LineFinder {
    fn get_points(&mut self, image: &opencv::core::Mat) -> Result<Vec<Point>, opencv::Error> {
        gaussian_blur(image, &mut self.blurred, Size::new(5, 5), 0.0, 0.0, BorderTypes::BORDER_CONSTANT.into());
        in_range(&self.blurred, &self.colour.low, &self.colour.high, &mut self.mask)?;
        ...
        Ok(points.iter().map(|p| {
            Point { pos: Pos { x: p.x as f64, y: p.y as f64}, confidence: time, point_type: self.line_type}
        }).collect())
    }
}

// use opencv vector or Vec?
fn perspective_correct(cv_points: Vec<opencv::core::Point>) -> Result<Vec<opencv::core::Point>, opencv::Error>{
...
impl Vision {
    ...
    pub fn update_points_from_image(&mut self, image: &opencv::core::Mat, point_map: &mut impl PointMap) -> () {
        ...
        for finder in &mut self.point_finders {
