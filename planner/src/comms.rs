...
const DEFAULT_DRIVE_COMMAND: messages::command::DriveCommand = messages::command::DriveCommand { state: messages::command::CommandType::StateOff as i32, throttle: 0., turn: 0.};
    ...
    pub fn start_accept_loop(&self){
        ...
        thread::spawn(move || {
            for stream in listener.incoming() {
                Self::start_recv_loop(stream.unwrap(), Arc::clone(&last_recieved), Arc::clone(&to_send));
            }
        });
    }

    pub fn start_recv_loop(mut stream: TcpStream, recieved_mutex: Arc<Mutex<messages::command::DriveCommand>>, to_send_mutex: Arc<Mutex<Box<messages::diagnostic::FullDiagnostic>>>) {
        ...
        thread::spawn(move || {
            ...
            loop{
                ...
                stream.write(&(to_send.encode_length_delimited_to_vec())).unwrap();
            }
        });
    }
}
...
impl Logger for NetworkComms {
    fn send_core(&mut self, message: &messages::diagnostic::FullDiagnostic){
        ...
        to_send.merge_length_delimited(&message.encode_length_delimited_to_vec()[..]).unwrap();
    }
}
